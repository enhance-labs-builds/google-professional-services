...
Create `frontend/src/app/common/prompts/your-feature.prompt.ts`. This is where the creative intelligence lives.
...
```typescript
// frontend/src/app/common/prompts/your-feature.prompt.ts
...
- **Proxy config**: `proxy.conf.json` routes `/api` to the Cloud Run backend — no CORS issues in dev
